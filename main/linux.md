...
4. 复制文件
   ...
   5. 打包或解压文件或目录
        ...
        -x：解开文件
         ...
         - 常用参数的组合
           ...
           - 格式：
             ...
             1. 打包（或压缩）：tar   参数  文件名   要打包|解压的文件目录
      
                例如： tar  -cvf   test.tar   ./*
           
             2. 解压：tar   参数  解压的文件名  -C  要打包|解压的文件目录
      ...
      6. 查找符合条件的字符串
         ...
         - grep   查找的字符串
         - grep   查找的字符串   -color     ------高亮显示查找的内容
         - grep    查找的字符串   -color  -A条数    -----查找内容的后几行也显示
         - grep    查找的字符串   -color  -B条数    -----查找内容的前几行也显示
   ...
   6. 将安装包从root目录下复制到/usr/local/下，并解压
...
- 查看mysql的服务状态  service  mysql  status
...
- mysqldump  -uroot -p123456  数据库名  >  备份地址（如：d:/1.sql）
- 还原 ： source   数据库文件
...
2. ##### 反向代理：
   ...
   - 反向代理：反向代理是指，用nginx服务器来代理目标服务器，客户端访问只能访问到nginx服务器，然后nginx分发请求到目标服务器，再接收服务器请求返回给客户端；
     ...
     ![](https://gss1.bdstatic.com/-vo3dSag_xI4khGkpoWK1HF6hhy/baike/c0%3Dbaike80%2C5%2C5%2C80%2C26/sign=4b1a652cc51349546a13e0363727f93d/3812b31bb051f81993dddee7d4b44aed2f73e7a7.jpg)
